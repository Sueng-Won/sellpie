...
//		String mEmail = session.getAttribute("user");
	...
	public String deleteFile(MultipartHttpServletRequest multipartHttpServletRequest, BoardVo board, String delfile, HttpServletRequest request){
		//세션정보생기면 바꾸기
		String email = "test2@naver.com";
		...
		if(null!=folder){
			...
			for(int x=0; x<a.length; x++){
				int result = boardService.deleteResource(folder+"/"+a[x].getName());
				if(0<result){
					for(int j=0; j<delfiles.length; j++){
						System.out.println("delfile : "+delfiles[j]);
						System.out.println("a[x].getName() : "+a[x].getName());
						if(delfiles[j].contains(a[x].getName())){
							a[x].delete();
						}else{
							ResourceVo resource = new ResourceVo();
							resource.setRsrc(savePath+"/"+a[x].getName());
							int rResult = resourceService.insertResource(resource);
							if(rResult<=0){
		...
		System.out.println("update에서 "+b.length);
